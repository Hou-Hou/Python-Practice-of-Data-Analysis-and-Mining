...
def getimgdir_designed(imgfilename):
    ...
    for root, dirs, files in os.walk(presentfiles):
        for afile in files:
            ...
            ffile = presentfiles + "\\" + afile
            aaa = afile.split('_')
            category.append(aaa[0])
            number.append(aaa[1].split('.')[0])
            print('leibie=',category)
            print('number=', number)
            if ffile[ffile.rindex("."):].lower() in imgTypes:
                imgdirs.append(ffile)
        if (not recursion):
            break
    return imgdirs, category, number
...
if __name__ == '__main__':
    ...
    waitcut, category, number = getimgdir_designed(doc)#注意：方式2、获取指定目录下的图片文件(不包含子文件夹的图片，仅当前文件夹）

    # 输入图片输出路径（若不存在，则新建目录）
    pic = '../data/pic_cut'
    # dstpath = saveimg(input('请输入图片输出目录：')) # 例如，输入：D:\PycharmProjects\readwriteFiles\img_cut_pix\pic_cut
    dstpath = saveimg(pic)
    ...
    df.insert(0, '水质类别', category)
    df.insert(1, '序号', number)
    df.to_csv('../data/type.csv',encoding='gbk',index=False)

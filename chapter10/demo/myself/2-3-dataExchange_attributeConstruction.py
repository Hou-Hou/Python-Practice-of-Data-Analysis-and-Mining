...
def step1():
    ...
    data1 = pd.read_excel('../my_data/data_guiyue.xlsx')
    x = pd.merge(data1, data[['事件编号']], left_index=True, right_index=True, how='outer')
    # 连接'data_guiyue.xlsx'和 'dataexchange_divideEvent.xlsx'的后两列,因为属性规约里面包含水流量为0的数值，
    # 后一个表中，只含有水流量不为0的值，需要将两边进行连接，获知规约后的数据中的数据所属的事件数,利用处理
    # 后的数开始进行数据构造工作
    print(x.head())
    x.to_excel('../my_data/data_for_attr_const.xlsx')
    ...
    df = pd.read_excel('../my_data/data_for_attr_const.xlsx')
    l = list(df['事件编号'])
    ...
    for j in range(MX):
        y = []
        for i in range(l.count(j+1)):   # l.count(j+1) 第j+1个事件的数量
            pos = l.index(j+1, pos+1)  # 返回事件编号为j+1的所有事件
            y.append(pos)
        a = min(y)           # 每个事件的序号值所在的最小的index值和最大的index值
        b = max(y)
        temp = df.iloc[a:b+1,:]
        Adf = pd.concat([Adf, temp])

    Adf['事件编号'].fillna(method='ffill', inplace=True)
    Adf['水流量'] = Adf[['水流量']].astype('float64')
    Adf.to_excel('../my_data/1TimeWaterDivide.xlsx')

# 第 2 步：构造属性
def step2():
    df2 = pd.read_excel('../my_data/1TimeWaterDivide.xlsx')
    df2['gap'] = df2[u'发生时间'].diff()
    MX = int(df2[u'事件编号'].max())  # 获取当前事件数172
    fdf2 = DataFrame([], index=range(1, MX + 1))  # 创建一个空列表 用来存放属性规约结果

    #  开始构造属性
    #  -------第一部分：用水事件开始编号、结束编号、用水开始时间、用水结束时间、
    #            用水总时间、用水间隔时间、一次用水期间“水流量”为0的记录数--------------
    ...
    # 第三部分：确定一次用水时间和停顿时间、总用水量、水流量波动、平均用户水量-----------------------------------------
    ...
    fdf2.rename(columns={'stDataIndex': u'起始数据编号', 'edDataIndex': u'终止数据编号', 'stUseTime': u'开始时间', 'enUseTime': u'结束时间',
                         'AllUseTime': u'总用水时长（w_time）', 'gapTimes': u'停顿次数（pause）', 'stopLines': u'停顿行数',
                         'AllUseTime(s)': u'所有使用时间总用水时长（s）', 'WashHour': u'洗浴时间点', 'UseTime': u'用水时长（use_water_time）',
                         'GapTime': u'总停顿时长（w_pause_time）', 'w_water': u'总用水量（w_water）',
                         'flow_volatility': u'水流量波动（flow_volatility）', 'water_rate': u'平均水流量（water_rate）',
                         'use_water_rate': u'用水/总时长（use_water_rate）', 'avg_pause_time': u'平均停顿时长（avg_pause_time）',
                         'pause_volatility': u'停顿时长波动（pause_volatility）'}, inplace='True')

    fdf2.index.name = u'事件编号'
    fdf2.to_excel('../my_data/attrConst_results.xlsx')

    # -----第 3 步-----接下来进行数据处理和筛选得到“候选洗浴事件，用于接下来的模型构建
    # 去掉  用水时长小于100秒
    # 去掉 总用水时长小于120秒
    # 去掉 一次用水事件中总用水量(纯热水)小于10升
    data_filter = fdf2[
        (fdf2[u'所有使用时间总用水时长（s）'] >= 120) & (fdf2[u'总用水量（w_water）'] >= 10) & (fdf2[u'用水时长（use_water_time）'] >= 100)]
    data_filter.to_excel('../my_data/data_filter.xlsx')
